...
설계 원칙(C): 역할은 pandas dtype이라는 객관적 사실로만 결정한다. 고유값 개수만으로
numeric을 categorical로 바꾸거나 target 후보를 추측하는 로직은 두지 않는다 — 제조
데이터에는 값 종류가 적은 discrete numeric 변수(예: 제어 설정값)가 정상적으로 존재하기
때문이다.
...
numeric이지만, 실행 옵션(`--target`)이 이를 target으로 지정했다면 분석상 역할은 "target"이다.
이 구분은 `profile_column()`이 아니라 target 목록을 아는
...
추론하는 것이 아니라, 실행 옵션이 명시한 target 지정을 그대로 반영할 뿐이다.
"""
...
RoleSource = Literal["dtype", "data_quality", "target_designation"]
...
def profile_column(
    ...
    """컬럼 하나의 프로파일을 만든다. 역할은 pandas dtype 기반으로만 결정한다."""
    n = len(series)
